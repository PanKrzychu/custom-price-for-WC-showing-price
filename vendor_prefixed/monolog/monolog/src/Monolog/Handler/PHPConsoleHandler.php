...
use CPWFreeVendor\PhpConsole\Connector;
use CPWFreeVendor\PhpConsole\Handler;
use CPWFreeVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \CPWFreeVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \CPWFreeVendor\PhpConsole\Connector $connector = null, $level = \CPWFreeVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('CPWFreeVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\CPWFreeVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \CPWFreeVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \CPWFreeVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\CPWFreeVendor\PhpConsole\Helper::isRegistered()) {
            \CPWFreeVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \CPWFreeVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \CPWFreeVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \CPWFreeVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \CPWFreeVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
