...
class ProductFields implements \CPWFreeVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    ...
    public function get_product_fields($product_object, $show_billing_period_options, $loop = \false)
    {
        ...
        $buy_pro_url = \get_locale() === 'pl_PL' ? 'https://www.wpdesk.pl/sklep/wlasna-cena-produktu-woocommerce-pro/?utm_source=wp-admin-plugins&utm_medium=link&utm_campaign=custom-price' : 'https://www.wpdesk.net/products/custom-price-for-woocommerce-pro/?utm_source=wp-admin-plugins&utm_medium=link&utm_campaign=custom-price';
        if (!\CPWFreeVendor\WPDesk\Library\CustomPrice\Integration::is_super()) {
            echo '<p class="form-field form-row full pro-description"><a href="' . \esc_url($buy_pro_url) . '">' . \esc_html__('Upgrade to PRO to enable additional features!', 'custom-price-for-woocommerce') . '</a></p>';
        }
