 ...
 */
class SimplePaidStrategy implements \CPWFreeVendor\WPDesk\Plugin\Flow\Initialization\InitializationStrategy
{
    ...
    public function run_init(\CPWFreeVendor\WPDesk_Plugin_Info $plugin_info)
    {
        ...
        \add_action('plugins_loaded', function () use($registrator) {
            $is_plugin_subscription_active = $registrator instanceof \CPWFreeVendor\WPDesk\License\PluginRegistrator && $registrator->is_active();
            if ($this->plugin instanceof \CPWFreeVendor\WPDesk\PluginBuilder\Plugin\ActivationAware && $is_plugin_subscription_active) {
    ...
    private function register_plugin()
    {
        if (\apply_filters('wpdesk_can_register_plugin', \true, $this->plugin_info)) {
            $registrator = new \CPWFreeVendor\WPDesk\License\PluginRegistrator($this->plugin_info);
