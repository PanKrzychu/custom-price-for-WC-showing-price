...
class WPDesk_Helper_List_Table extends \WP_List_Table
{
    ...
    {
        echo \wpautop(\__('No WP Desk plugins found.', 'custom-price-for-woocommerce'));
    }
    ...
    {
        $columns = ['product_name' => \__('Plugin', 'custom-price-for-woocommerce'), 'product_status' => \__('Subscription Status', 'custom-price-for-woocommerce'), 'product_license' => \__('Subscription Data', 'custom-price-for-woocommerce')];
        return $columns;
    }
    ...
    {
        $status = \__('Deactivated', 'custom-price-for-woocommerce');
        if ($item['activation_status'] == 'Activated') {
            $status = \__('Activated', 'custom-price-for-woocommerce');
        }
        return $status;
    }
